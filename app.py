...
# State-wise average annual irradiance (kWh/m²/yr)
STATE_IRRADIANCES = {
...
# Electricity tariff (₹/kWh) for all states and UTs (approximate)
STATE_TARIFFS = {
    ...
    "Andaman and Nicobar Islands": 6.5, "Chandigarh": 7.0, "Dadra and Nagar Haveli and Daman and Diu": 7.0,
    "Lakshadweep": 6.5, "Delhi": 8.0
}
...
# Default roof area by house type if OSM fails (m²)
HOUSE_TYPE_AREA = {
...
def compute_area(geojson_polygon):
    """Compute polygon area in square meters using correct projection."""
    geom = shape(geojson_polygon)
    transformer = Transformer.from_crs(CRS.from_epsg(4326), CRS.from_proj4(
...
def get_pvgis_irradiance(lat, lon):
    """Fetch annual irradiance (GHI) from PVGIS."""
...
def calculate_results(area, shadow_area, irradiance, orientation_factor, tariff):
    ...
    capacity_kw = effective_area / 10  # 1 kW ≈ 10 m²
    ...
    if lat and lon:
        ...
        if area_method == "Get from address":
            ...
            if poly_area:
                ...
                st.warning("No building footprint found. Please select house type for default roof area.")
                house_type = st.selectbox("Select house type for default area:", list(HOUSE_TYPE_AREA.keys()))
                roof_area = HOUSE_TYPE_AREA.get(house_type, 100)
                st.info(f"Using default roof area for {house_type}: {roof_area} m²")
    else:
        st.error("Could not geocode address. Please select house type for default roof area.")
