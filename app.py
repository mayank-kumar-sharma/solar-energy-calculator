...
def get_building_polygon(lat, lon):
    ...
    Returns polygon area in m² if found, else None.
    """
...
def compute_area(geojson_polygon):
    ...
    area_m2 = transform(proj, geom).area
    return area_m2
...
def get_pvgis_irradiance(lat, lon):
    ...
    try:
        data = r.json()
        # PVGIS gives annual data in outputs.totals.fixed.E_y
...
if area_method == "Enter directly":
    ...
    if address:
        ...
        if lat and lon:
            st.success(f"Geocoded location: {location_name} ({lat:.4f}, {lon:.4f})")
            poly_area = get_building_polygon(lat, lon)
            if poly_area:
                roof_area = poly_area
                st.info(f"Detected building area: {roof_area:.2f} m²")
            else:
                # Default if no polygon found
                roof_area = 100.0
...
# Run calculation
if roof_area and (lat and lon or area_method == "Enter directly"):
    if lat and lon:
        irradiance = get_pvgis_irradiance(lat, lon)
    else:
        irradiance = 1700  # default average if no location
        st.warning("No location provided, using default irradiance 1700 kWh/m²/yr.")
    ...
    if irradiance:
        ...
        st.error("Could not fetch irradiance data from PVGIS.")
